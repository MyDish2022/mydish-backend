...
class UserService {
  ...
  async verifyPassword({ password }, user) {
    try {
      ...
      const verifyIfPasswordTheSame = await bcrypt.compare(
        password.toString(),
        user.password
      );
    ...
    try {
      ...
      if (!user)
        throw new AuthorizationError(
          "you must sign in in order to add your restaurant!!"
        );
      const restaurantExist = await RestaurantModel.findOne({ login }).lean();
      if (restaurantExist)
        throw new AlreadyExistError("Restaurant mail already exist!!");
      let characters =
